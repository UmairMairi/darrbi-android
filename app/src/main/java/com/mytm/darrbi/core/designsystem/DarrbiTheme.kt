 ...
 * [DarrbiTheme] (never hardcoded). Typography is locale-aware (Madani for Arabic / SF Pro otherwise) and is
 * rebuilt from [appFontFamily] each composition. Also derives a Material3 `ColorScheme` for M3 components.
 */
val LocalDarrbiColors = staticCompositionLocalOf { LightDarrbiColors }
val LocalDarrbiTypography = staticCompositionLocalOf { darrbiTypography(SfProFontFamily) }
